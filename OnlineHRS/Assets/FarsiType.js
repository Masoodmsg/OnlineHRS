...
FarsiType.init = function (window) {

    //var Inputs = document.getElementsByTagName('INPUT');
    var Inputs = window.getEl().select('.farsi').elements;
    ...
    for (var i = 0; i < items.length; i++) {
        if (items[i].xtype == 'textfield' && items[i].cls == 'eng') {
            var input = items[i].getEl().select('input').elements[0];
            //input.onkeypress = setEnglishType;
            items[i].enableKeyEvents = true;
            items[i].initEvents();
            items[i].addListener('keypress', function (item, event) {
                ...
                //check windows lang
                if (key > 128) {
                    //if (item.callout) { item.callout.toggle(); return false; }
                    callout = Ext.Msg.callout(item, { alignment: 'left', html: 'کیبورد فارسی است.<br/>این فیلد نمی تواند فارسی باشد.', ui: 'danger', rtl: "true", title: 'هشدار' });
...
function setEnglishType(e) {
    ...
    if (key > 128) {
        alert('wfwfwf');
        textField.callout = Ext.Msg.callout(textField, { alignment: 'lefttop', html: 'LeftTop callout', height: 100, width: 100 });
        return false;
    }
}
